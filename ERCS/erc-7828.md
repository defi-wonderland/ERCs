...
#### bip122
bip122 chains are distinguished by the block hash of where they forked from bitcoin, or by their genesis blockhash if that does not apply (as is the case for bitcoin itself)
This  blockhash is to be stored raw in the binary representation, without encoding/decoding from/to base58btc or truncating it, and without removing any leading zeroes.

As for the human-readable name, the lowercase-hexadecimal representation of the 32 bytes should be used.
...
: `bip122:12a765e31ffd4059bada1e25190f6e98c99d9714d334efa41a195a7e7e04bfe2`

#### solana
Similarly to bip122, Solana networks are distinguished by its genesis blockhash, which is to be decoded from base58btc and stored raw in the binary representation and without removing any leading zeroes:
...
### Previous versions of ERC-7828
The scope of this standard has mutated from a text-only representation of addresses with optional resolution into more human-readable strings by use of existing ENS infrastructure for address names, and both the centralized ethereum-lists/chains list and ERC-7785 for resolution of chain names, to the current scope of a simple but extendable binary-first format.
...
## Future work
- Coordination with CASA to maintain the serialization schemes kickstarted by appendixes A and B
- Development of a standard defining ways for Interoperable Addresses to be condensed into more human readable names with the help of naming registries.
